...
"""
Qwen-Image rollout adapter for DiffusionNFT algorithm.
...
__all__ = ["QwenImageNFTPipelineWithLogProb"]


@VllmOmniPipelineBase.register("QwenImagePipeline", algorithm="nft")
class QwenImageNFTPipelineWithLogProb(QwenImagePipelineWithLogProb):
    """Rollout pipeline for Qwen-Image under the NFT algorithm.

    Inherits from the FlowGRPO rollout adapter but defaults `logprobs=False`
    to skip unnecessary per-step log-probability computation. The NFT training
    loop only uses `all_latents[:, -1]` (the final clean latent) — intermediate
