...
"""CPU tests for QwenImageEditPlus training adapters (FlowGRPO and NFT)
plus the rollout-side ``_get_qwen_prompt_embeds`` visual-feature wiring.
...
from verl_omni.pipelines.qwen_image_edit_nft.diffusers_training_adapter import QwenImageEditPlusNFT
...
class TestQwenImageEditPlusRegistry:
    ...
    def test_registered_for_nft(self):
        cfg = _make_model_config(algorithm="nft")
        assert DiffusionModelBase.get_class(cfg) is QwenImageEditPlusNFT

    def test_nft_is_subclass_of_flow_grpo(self):
        assert issubclass(QwenImageEditPlusNFT, QwenImageEditPlus)
...
# ===========================================================================
# 4. QwenImageEditPlusNFT — NFT adapter specifics
# ===========================================================================


class TestQwenImageEditPlusNFT:
    def test_registered_for_nft_algorithm(self):
        cfg = _make_model_config(algorithm="nft")
        assert DiffusionModelBase.get_class(cfg) is QwenImageEditPlusNFT

    def test_forward_and_sample_previous_step_raises_not_implemented(self):
        """NFT uses forward-process loss, so reverse-SDE log-prob must never be called."""
        with pytest.raises(NotImplementedError, match="forward_and_sample_previous_step"):
            QwenImageEditPlusNFT.forward_and_sample_previous_step(
                module=MagicMock(),
                scheduler=MagicMock(),
                model_config=_make_model_config(algorithm="nft"),
    ...
    def test_inherits_prepare_model_inputs_from_flow_grpo(self):
        """NFT reuses the FlowGRPO adapter's prepare_model_inputs unchanged."""
        tensors = _batch_tensors()
        micro_batch = _make_micro_batch(tensors)
        step = 0

        model_inputs, _ = QwenImageEditPlusNFT.prepare_model_inputs(
            module=MagicMock(),
            model_config=_make_model_config(algorithm="nft"),
            latents=tensors["all_latents"],
            timesteps=tensors["all_timesteps"],
        ...
        # Condition image latents should be concatenated just as in FlowGRPO
        assert model_inputs["hidden_states"].shape == (_B, _LS + _CS, _LC)


# ===========================================================================
# 5. Rollout adapter — _get_qwen_prompt_embeds visual-feature wiring
# ===========================================================================
