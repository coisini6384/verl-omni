#!/usr/bin/env bash
# DiffusionNFT e2e smoke test (minimal runtime), vllm_omni rollout.
#
# Exercises the full NFT pipeline:
#   parquet load -> vllm_omni rollout (final latents only) -> visual reward
#   (jpeg_compressibility, no reward model) -> NFT per-timestep training
#   -> FSDP LoRA -> sync.
#
...
NUM_GPUS=${NUM_GPUS:-1}
MODEL_PATH=${MODEL_PATH:-${./pretrained_models/qwen-image}}
...
# ── DiffusionNFT: no reward model (jpeg_compressibility rule reward) ─────────
python3 -m verl_omni.trainer.main_diffusion \
    ...
    algorithm.nft_num_train_timesteps=2 \
    ...
    trainer.experiment_name=nft-diffusion-e2e \
...
echo "DiffusionNFT e2e test passed (training completed successfully)."
